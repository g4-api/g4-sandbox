...
function Get-LiteLLMProxyProcess {
    ...
    $process = Get-CimInstance -ClassName Win32_Process -Filter "ProcessId = $proxyPid" -ErrorAction SilentlyContinue
    if ($null -eq $process) {
        Remove-Item -LiteralPath $Context.ProxyProcessStatePath -Force -ErrorAction SilentlyContinue
        return $null
    }

    $expectedPython = Get-PortablePythonExe -Context $Context
    $expectedBootstrap = [System.IO.Path]::GetFullPath($Context.LiteLLMBootstrapPath)
    $actualPython = [string]$process.ExecutablePath
    $commandLine = [string]$process.CommandLine

    $identityMatches = -not [string]::IsNullOrWhiteSpace($actualPython) -and
        [string]::Equals(
            [System.IO.Path]::GetFullPath($actualPython),
            [System.IO.Path]::GetFullPath($expectedPython),
            [System.StringComparison]::OrdinalIgnoreCase) -and
        -not [string]::IsNullOrWhiteSpace($commandLine) -and
        $commandLine.IndexOf($expectedBootstrap, [System.StringComparison]::OrdinalIgnoreCase) -ge 0
...
function Stop-LiteLLMProxy {
    ...
    while ($pending.Count -gt 0) {
        ...
        $children = @($allProcesses | Where-Object { [int]$_.ParentProcessId -eq $parentPid })
        foreach ($child in $children) {
            $childPid = [int]$child.ProcessId
            if ($processTree -notcontains $childPid) {
                $processTree += $childPid
                $pending += $childPid
            }
        }
    }
    ...
    while ($null -ne (Get-Process -Id $proxyPid -ErrorAction SilentlyContinue) -and [DateTime]::UtcNow -lt $deadline) {
        Start-Sleep -Milliseconds 200
    }

    if ($null -ne (Get-Process -Id $proxyPid -ErrorAction SilentlyContinue)) {
        throw [System.InvalidOperationException]::new(
...
function Test-LiteLLMDatabaseProvisioned {
    ...
    # Reports whether the LiteLLM role AND database already exist in the cluster's catalogs.
    # A clean deploy never creates them - the first Start does - so verification and Start
    # provisioning must tell "provisioned" apart from "not", rather than assuming a bare
    # role connectivity check is meaningful.
    $psqlExe = Join-Path $Context.PgBin 'psql.exe'
    if (-not (Test-Path -LiteralPath $psqlExe)) {
        return $false
    }
...
function Assert-PrismaClientUsable {
    ...
    if ((Test-PostgresReady -Context $Context) -and (Test-LiteLLMDatabaseProvisioned -Context $Context)) {
        ...
        $checkName = 'construct (PostgreSQL not running or database not yet provisioned; connect check runs on first Start)'
    }
...
function Test-PortableDeployment {
    ...
    # Stage: database connectivity as the LiteLLM role. Only meaningful once the role and
    # database exist - a clean deploy leaves them to the first Start.
    if (Test-LiteLLMDatabaseProvisioned -Context $Context) {
        $psqlExe = Join-Path $Context.PgBin 'psql.exe'
        Invoke-NativeCommand -FilePath $psqlExe -ArgumentList @(
            '-h', $Context.PostgresHostAddress,
        ...
        Write-Host '[OK] LiteLLM database not yet provisioned - role and database are created on the first Start.'
    }
