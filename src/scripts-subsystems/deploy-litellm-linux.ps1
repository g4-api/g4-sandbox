...
exec pwsh -NoProfile -File "${here}/__SCRIPT__" -Action Start "$@"
'@

    $startCommand = $startCommandTemplate.Replace('__SCRIPT__', $Context.ScriptFileName)
    Write-BoxShellScript -Path $Context.StartCommandPath -Content $startCommand

    Write-Host '[OK] Portable helper files and start-litellm.sh created.'
}
