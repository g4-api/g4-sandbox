<#
.SYNOPSIS
    Deploys, updates, and manages a fully portable, self-contained LiteLLM proxy stack on Linux.

.DESCRIPTION
    deploy-litellm-linux.ps1 is the Linux counterpart of deploy-litellm-win.ps1. It runs under
    PowerShell 7 and is written in Windows PowerShell 5.1-compatible syntax.
    ...
    Prisma Node toolchain, and PostgreSQL (portable glibc binaries from
    theseus-rs/postgresql-binaries). No machine-level, user-level, shell-profile, or systemd
    ...
    ----------
      box/
        ...
        bin/                        generated helper scripts (version-agnostic)
        home/  temp/
        runtimes/
    ...
    prisma (Python) is not hard-coded either: the exact version is read from LiteLLM's own
    uv.lock at tag v<LiteLLMVersion> - the version LiteLLM's Docker image and CI test with -
    because LiteLLM only declares a range in pyproject.toml and keeps prisma in its
    "extra-proxy" extra (so litellm[proxy] does not pull it, and resolving the range gets the
    newest rather than the tested one). Its internal Prisma CLI + engine defaults are then left
    in force (PRISMA_VERSION unset), and the nodeenv Node.js version defaults to 20. Pass
    -PrismaPythonVersion / -PrismaCliVersion / -PrismaEngineVersion / -PrismaNodeVersion to
    override; -PrismaPythonFallbackVersion covers an offline uv.lock lookup.
...
.PARAMETER PrismaPythonVersion
    ...
    version is read from LiteLLM's own uv.lock at tag v<LiteLLMVersion> - the version LiteLLM's
    Docker image and CI are built and tested with. (LiteLLM only declares a range,
    "prisma>=0.11.0,<1.0", in pyproject.toml, and it keeps prisma in the "extra-proxy" extra so
    litellm[proxy] does not pull it - resolving the range gets the newest, not the tested one.)

.PARAMETER PrismaPythonFallbackVersion
    prisma (Python client) version used only when the uv.lock lookup fails (offline, tag or
    file missing, unparseable). Default 0.11.0 - LiteLLM's long-standing floor. Not the primary
    path; a loud warning is printed when it is used.
...
.PARAMETER PostgresVersion
    theseus-rs/postgresql-binaries release tag (PostgreSQL version, for example 17.11.0).
...
.PARAMETER PostgresSuperUser
    Superuser role used to initialize the PostgreSQL cluster and to create the LiteLLM role and
    database. initdb always creates it (deterministic across machines) and psql uses it for role
    and database creation. Default 'postgres'.
...
.PARAMETER UvDownloadUrl
    Override for the uv release archive URL. Derived from UvVersion and the host architecture.

.PARAMETER PostgresDownloadUrl
    Override for the PostgreSQL binary archive URL. Derived from PostgresVersion and the host
    architecture.
...
.EXAMPLE
    pwsh -File ./deploy-litellm-linux.ps1
    Deploy the default LiteLLM version into the current box directory.

.EXAMPLE
    pwsh -File ./deploy-litellm-linux.ps1 -Action Update -LiteLLMVersion 1.99.0
    Build a runtime for 1.99.0, migrate the database, and switch to it. 1.98.0 is kept.

.EXAMPLE
    pwsh -File ./deploy-litellm-linux.ps1 -Action Rollback
    Switch back to the previously active runtime.

.EXAMPLE
    pwsh -File ./deploy-litellm-linux.ps1 -Action Status
...
.NOTES
    Target host : PowerShell 7 on glibc Linux (x86_64 or aarch64).
    Privileges  : Run as a normal user. PostgreSQL initdb and the server refuse to run as root.
    Isolation   : No global install of Python, Node, Prisma, PostgreSQL, or uv.
    PostgreSQL  : Portable glibc binaries from theseus-rs/postgresql-binaries.
    Redis       : Omitted. LITELLM_DISABLE_NO_REDIS_WARNING=true (single-worker profile).
    MCP support : The mcp package loads natively on Linux (no pywin32). litellm-portable.py
                  still processes the .pth files under the runtime's packages directory.
    Not deployed: vLLM. The default config expects http://127.0.0.1:8000/v1 serving Qwen/Qwen3-0.6B.
#>
...
param(
    ...
    [Parameter()]
    [ValidateNotNullOrEmpty()]
    [string]$PostgresVersion = '17.11.0',
...
# This script validates every native exit code explicitly (Invoke-NativeCommand, the
# -PassThruExitCode probes). Opt out of PowerShell 7.4+ turning a non-zero native exit into a
# terminating error so those checks - and the readiness probes that expect non-zero - work.
...
$script:SharedLayoutDirectories = @('data', 'state', 'cache', 'cache/uv', 'cache/litellm-locks', 'bin', 'home', 'temp', 'runtimes')

# Directories created inside a runtime slug directory. prisma/nodeenv is owned by the pre-seed.
$script:RuntimeLayoutDirectories = @('python', 'packages', 'prisma', 'prisma/binaries', 'prisma/npm')
...
$script:RuntimeVerifyExpression = "import os,site; site.addsitedir(os.environ['PYTHONPATH']); import importlib.metadata as m, prisma; print('LiteLLM=' + m.version('litellm')); print('Prisma=' + prisma.__version__)"

$script:RuntimeImportExpression = "import importlib.metadata as m; import litellm, prisma; print('LiteLLM=' + m.version('litellm')); print('Prisma=' + prisma.__version__); print('LiteLLMPath=' + litellm.__file__)"
...
function Test-MuslLibc {
    [CmdletBinding()]
    [OutputType([bool])]
    param()

    # The ld-musl loader is the most reliable musl signal; glibc distributions never ship it.
    foreach ($libraryDirectory in @('/lib', '/usr/lib')) {
        if (-not (Test-Path -LiteralPath $libraryDirectory)) {
            continue
        }

        $muslLoaders = @(Get-ChildItem -LiteralPath $libraryDirectory -Filter 'ld-musl-*' -File -ErrorAction SilentlyContinue)

        if ($muslLoaders.Count -gt 0) {
            return $true
        }
    }
...
function Get-LinuxTargetTriple {
    ...
    # Stage: architecture selection. The bundled payloads are published per Rust target triple.
    $machine = (& uname -m)

    if ($LASTEXITCODE -ne 0) {
        throw [System.InvalidOperationException]::new('Could not determine the host architecture via uname -m.')
    }

    $normalizedMachine = "$machine".Trim().ToLowerInvariant()

    switch ($normalizedMachine) {
        'x86_64' { return 'x86_64-unknown-linux-gnu' }
        'amd64' { return 'x86_64-unknown-linux-gnu' }
        'aarch64' { return 'aarch64-unknown-linux-gnu' }
        'arm64' { return 'aarch64-unknown-linux-gnu' }
        default {
            throw [System.PlatformNotSupportedException]::new(
                "Unsupported architecture '$machine'. This deployment requires an x86_64 or aarch64 Linux host."
            )
        }
    }
}

function Assert-LinuxPlatform {
    [CmdletBinding()]
    param()

    # Stage: guard. Every runtime this script installs is a glibc Linux binary.
    $isLinuxHost = $false
    $isLinuxVariable = Get-Variable -Name 'IsLinux' -ErrorAction SilentlyContinue

    if ($null -ne $isLinuxVariable) {
        $isLinuxHost = [bool]$isLinuxVariable.Value
    }

    if (-not $isLinuxHost) {
        throw [System.PlatformNotSupportedException]::new(
            'deploy-litellm-linux.ps1 supports Linux only. Use deploy-litellm-win.ps1 on Windows.'
        )
    }

    # Stage: privilege guard. initdb and postgres refuse to run as the root user.
    $currentUserId = (& id -u)

    if ($LASTEXITCODE -eq 0 -and "$currentUserId".Trim() -eq '0') {
        throw [System.InvalidOperationException]::new(
            'Run this script as a normal (non-root) user. PostgreSQL initdb and the server refuse to run as root.'
        )
    }

    # Stage: libc guard. The downloaded binaries are built for glibc.
    if (Test-MuslLibc) {
        throw [System.PlatformNotSupportedException]::new(
            'musl libc (for example Alpine) is not supported. This deployment requires a glibc-based Linux distribution.'
        )
    }
}
...
function Get-RuntimeSlug {
    ...
    # Slug names the runtime directory and is safe as a path segment for real version strings.
...
function Read-ActiveRuntimeState {
    ...
    # Normalize: a state object missing any expected key is treated as absent, so callers can
    # read .active / .previous / .history directly under strict mode.
...
function Write-ActiveRuntimeState {
    ...
    $statePath = Join-Path $Root $script:ActiveRuntimeStateRelativePath
    Write-BoxTextFile -Path $statePath -Content ($stateObject | ConvertTo-Json -Depth 5)
}
...
function New-PortableContext {
    ...
    $scriptFileName = 'deploy-litellm-linux.ps1'
    ...
    if ([string]::IsNullOrWhiteSpace($resolvedUvDownloadUrl)) {
        $resolvedUvDownloadUrl = "https://github.com/astral-sh/uv/releases/download/$UvVersion/uv-$TargetTriple.tar.gz"
    }

    $resolvedPostgresDownloadUrl = $PostgresDownloadUrl
    if ([string]::IsNullOrWhiteSpace($resolvedPostgresDownloadUrl)) {
        $resolvedPostgresDownloadUrl = "https://github.com/theseus-rs/postgresql-binaries/releases/download/$PostgresVersion/postgresql-$PostgresVersion-$TargetTriple.tar.gz"
    }
    ...
    return [pscustomobject]@{
        ...
        PrismaClientPyShimPath   = Join-Path $binDirectory 'prisma-client-py'
        StartCommandPath         = Join-Path $root 'start-litellm.sh'
        StopCommandPath          = Join-Path $root 'stop-litellm.sh'
        ...
        PrismaCliPath            = $(if ($null -ne $prismaCacheDirectory) { Join-Path $prismaCacheDirectory 'binaries/node_modules/.bin/prisma' } else { $null })
        PrismaNodeExe            = $(if ($null -ne $prismaCacheDirectory) { Join-Path $prismaCacheDirectory 'nodeenv/bin/node' } else { $null })
        PrismaCliIndexJs         = $(if ($null -ne $prismaCacheDirectory) { Join-Path $prismaCacheDirectory 'binaries/node_modules/prisma/build/index.js' } else { $null })
        PrismaQueryEnginePath    = $(if ($null -ne $prismaCacheDirectory) { Join-Path $prismaCacheDirectory 'query-engine' } else { $null })
...
function Get-PortablePythonExe {
    ...
    # Stage: resolution. uv names its managed CPython directory with the libc suffix on Linux,
    # so discover the interpreter by pattern instead of hard-coding the folder name.
    $candidates = @()
    ...
    if ($null -ne $Context.PythonInstallDir -and (Test-Path -LiteralPath $Context.PythonInstallDir)) {
        $candidates = @(
            Get-ChildItem -LiteralPath $Context.PythonInstallDir -Directory -Filter 'cpython-*' -ErrorAction SilentlyContinue |
                ForEach-Object { Join-Path $_.FullName 'bin/python3' } |
...
function Set-PortableProcessEnvironment {
    ...
    # npm (Prisma toolchain phase) fetch resilience. npm reads lowercase npm_config_* vars.
    ...
    if ($null -ne $Context.RuntimeDir) {
        ...
        # Prisma CLI + engine pin. Set only when known (manifest or override); the runtime
        # build derives and sets these before they are needed. Never set to an empty string:
        # prisma-client-python reads an empty PRISMA_VERSION as "install prisma@latest".
    ...
    # Stage: isolation. Replace PATH with the box binaries plus the minimum system directories.
    $isolatedPathEntries = @(
        $Context.BinDir,
        $Context.PgBin,
        '/usr/bin',
...
function Write-BoxShellScript {
    ...
    # Stage: filesystem write. LF, trailing newline, BOM-free UTF-8, executable bit.
    $normalizedContent = ($Content -replace "`r`n", "`n")
    ...
    if ($LASTEXITCODE -ne 0) {
        throw [System.InvalidOperationException]::new("Could not mark script executable: $Path")
    }
}
...
function Invoke-SystemCommand {
    ...
    # Stage: process call. Run a standard system tool resolved through the isolated PATH.
    & $FileName @ArgumentList | Out-Host
    $observedExitCode = $LASTEXITCODE

    if ($observedExitCode -ne 0) {
        throw [System.InvalidOperationException]::new("System command '$FileName' failed with exit code $observedExitCode.")
    }
}
...
function Set-SecurityProtocol {
    ...
    # Windows PowerShell 5.1 (Desktop) needs TLS 1.2 enabled; PowerShell 7 already negotiates it.
...
function Expand-DownloadedArchive {
    ...
    # Every Linux payload is a gzip-compressed tarball.
    Invoke-SystemCommand -FileName 'tar' -ArgumentList @('-xzf', $ArchivePath, '-C', $DestinationPath)
}
...
function Test-PathWithinContainer {
    ...
    return $normalizedCandidate.StartsWith($normalizedContainer, [System.StringComparison]::Ordinal)
}
...
function Remove-ContainedDirectory {
    ...
    $isSymbolicLink = ($targetItem.Attributes -band [System.IO.FileAttributes]::ReparsePoint) -eq [System.IO.FileAttributes]::ReparsePoint

    if ($isSymbolicLink) {
        throw [System.InvalidOperationException]::new("Refusing to remove a symbolic link: $TargetPath")
    }
...
# Python does not process .pth files from PYTHONPATH directories, and some
# LiteLLM dependencies ship them, so process them explicitly here.
# ---------------------------------------------------------------------------
...
if [ -z "${PORTABLE_PYTHON_EXE:-}" ] || [ ! -x "${PORTABLE_PYTHON_EXE}" ]; then
    echo "PORTABLE_PYTHON_EXE is not set to an executable interpreter" >&2
    exit 1
fi

exec "${PORTABLE_PYTHON_EXE}" -m prisma "$@"
'@

    Write-BoxShellScript -Path $Context.PrismaClientPyShimPath -Content $prismaClientPyShim
    ...
    # isolation, and PostgreSQL process control - no install/build logic. start-litellm.sh and
    # stop-litellm.sh (below) call these directly, so the box never depends on
    # deploy-litellm-linux.ps1 being present after the deploy that built it.
    $runtimeCommonScript = @'
...
function Get-ActiveRuntimeSlug {
    ...
    throw [System.InvalidOperationException]::new('No active runtime found. Deploy the box first with deploy-litellm-linux.ps1 -Action Deploy.')
}
...
function Test-PostgresReady {
    ...
    $pgIsReadyExe = Join-Path $Context.PgBin 'pg_isready'
...
function Invoke-PgCtl {
    ...
    $pgCtlExe = Join-Path $Context.PgBin 'pg_ctl'
...
exec pwsh -NoProfile -File "${here}/bin/litellm-start-runtime.ps1" "$@"
'@

    Write-BoxShellScript -Path $Context.StartCommandPath -Content $startCommandTemplate
...
here="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
exec pwsh -NoProfile -File "${here}/bin/litellm-stop-runtime.ps1" "$@"
'@

    Write-BoxShellScript -Path $Context.StopCommandPath -Content $stopCommandTemplate

    Write-Host '[OK] Portable helper files, start-litellm.sh and stop-litellm.sh created.'
}
...
function Install-Uv {
    ...
    $archivePath = Join-Path $Context.TempDir "uv-$stagingToken.tar.gz"
    ...
    try {
        ...
        $uvExecutable = Find-ArchiveExecutable -SearchRoot $extractPath -FileName 'uv'
        $sourceDirectory = $uvExecutable.Directory.FullName

        foreach ($toolName in @('uv', 'uvx')) {
            $sourceFile = Join-Path $sourceDirectory $toolName

            if (Test-Path -LiteralPath $sourceFile) {
                Invoke-SystemCommand -FileName 'cp' -ArgumentList @('-a', $sourceFile, $Context.Root)
            }
        }
    }
    finally {
    ...
    if (-not (Test-Path -LiteralPath $Context.UvExe)) {
        throw [System.IO.FileNotFoundException]::new("uv was not present after extraction: $($Context.UvExe)")
    }

    Invoke-SystemCommand -FileName 'chmod' -ArgumentList @('+x', $Context.UvExe)
    Invoke-NativeCommand -FilePath $Context.UvExe -ArgumentList @('--version')
}
...
function Install-PortablePython {
    ...
    Invoke-NativeCommand -FilePath $Context.UvExe -ArgumentList @('python', 'install', $Context.PythonVersion, '--no-bin')
...
function Get-PythonOutputLines {
    ...
    # Runs "python -c <expr>" and returns its stdout lines. Callers MUST wrap in @(...):
    # PowerShell unrolls a 0- or 1-element return to $null / a scalar.
    try {
        $output = & $PythonExe -c $Expression 2>$null
        if ($LASTEXITCODE -ne 0) {
            return @()
        }
...
function Get-LiteLLMLockedPackageVersion {
    ...
    foreach ($template in $script:LiteLLMLockUrlTemplates) {
        ...
        foreach ($ref in $refs) {
            ...
            # Cached file did not contain the package; drop it so a later ref can be tried.
            Remove-Item -LiteralPath $cachePath -Force -ErrorAction Ignore
        }
    }
...
function Resolve-PrismaPins {
    ...
    # Stage: version derivation.
    #  - Prisma CLI version + engine hash: left UNSET unless the caller overrides. An unset
    #    PRISMA_VERSION lets prisma-client-python use its own internal default, which is always
    #    the exact CLI + engine that release was built against - the correct value by
    #    construction. (We only ever set these to defeat a stray value, never to guess one.)
    #  - Node.js: every prisma-client-python release (0.11 - 0.15, the whole unmaintained line)
    #    pins a Prisma 5.x CLI, which supports Node 20. The Node 22 branch only triggers on an
    #    explicit -PrismaCliVersion >= 6 override.
    $prismaClientVersion = ''
...
function Install-LiteLLMRuntime {
    ...
    if ((Test-Path -LiteralPath $liteLlmDistInfo) -and (Test-Path -LiteralPath $prismaDistInfo)) {
        Write-Host "[OK] LiteLLM $($Context.LiteLLMVersion) + prisma $PrismaVersion already installed for $($Context.RuntimeSlug)."
    }
    else {
        ...
        Write-Host "[INSTALL] LiteLLM $($Context.LiteLLMVersion) + prisma $PrismaVersion"
...
function Install-PrismaToolchain {
    ...
    $queryEngineCandidates = @(
        Get-ChildItem `
            -Path (Join-Path $Context.PrismaBinaryCacheDir 'node_modules/prisma/query-engine-*') `
            -File `
            -ErrorAction SilentlyContinue
        Get-ChildItem `
            -Path (Join-Path $Context.PrismaBinaryCacheDir 'node_modules/@prisma/engines/query-engine-*') `
            -File `
            -ErrorAction SilentlyContinue
    )

    $queryEngineSource = $null
    foreach ($candidate in @($queryEngineCandidates | Sort-Object FullName)) {
        try {
            & $candidate.FullName --version 2>$null | Out-Null
            if ($LASTEXITCODE -eq 0) {
                $queryEngineSource = $candidate.FullName
                break
            }
        }
        catch {
    ...
    if ($null -eq $queryEngineSource) {
        throw [System.IO.FileNotFoundException]::new(
            "No executable Prisma query engine was found under '$($Context.PrismaBinaryCacheDir)/node_modules'.")
    }

    Copy-Item -LiteralPath $queryEngineSource -Destination $Context.PrismaQueryEnginePath -Force
    Invoke-NativeCommand -FilePath '/bin/chmod' -ArgumentList @('+x', $Context.PrismaQueryEnginePath)
...
function Install-PortablePostgres {
    ...
    $postgresExePath = Join-Path $Context.PgBin 'postgres'
    ...
    $stagingToken = [System.Guid]::NewGuid().ToString('N')
    $archivePath = Join-Path $Context.TempDir "postgresql-$stagingToken.tar.gz"
    $extractPath = Join-Path $Context.TempDir "postgresql-extract-$stagingToken"

    try {
        Save-RemoteFile -Uri $Context.PostgresDownloadUrl -DestinationPath $archivePath -ChecksumUri ($Context.PostgresDownloadUrl + '.sha256')
        Expand-DownloadedArchive -ArchivePath $archivePath -DestinationPath $extractPath

        $directoryFilter = { $_.Directory.Name -ieq 'bin' }
        $postgresExecutable = Find-ArchiveExecutable -SearchRoot $extractPath -FileName 'postgres' -DirectoryFilter $directoryFilter
        ...
        Invoke-SystemCommand -FileName 'cp' -ArgumentList @('-a', ($distributionRoot + '/.'), $Context.PgHome)
    }
    finally {
        Remove-Item -LiteralPath $archivePath -Force -ErrorAction Ignore
        Remove-Item -LiteralPath $extractPath -Recurse -Force -ErrorAction Ignore
    ...
    if (-not (Test-Path -LiteralPath $postgresExePath)) {
        throw [System.IO.FileNotFoundException]::new("postgres was not found after extraction: $postgresExePath")
    }
...
function Initialize-PostgresCluster {
    ...
    Add-Content -LiteralPath (Join-Path $Context.PgData 'postgresql.conf') -Value $configLines -Encoding utf8
...
function Invoke-PgCtl {
    ...
    # pg_ctl start launches a detached postgres that holds pg_ctl's inherited output handles for
    # the life of the server, so a pumping wait never returns. Redirect streams and wait only on
    # the pg_ctl process object itself.
    $stdoutPath = Join-Path $Context.TempDir ('pg_ctl-out-' + [System.Guid]::NewGuid().ToString('N') + '.log')
    $stderrPath = Join-Path $Context.TempDir ('pg_ctl-err-' + [System.Guid]::NewGuid().ToString('N') + '.log')
...
function Invoke-RuntimeUpdate {
    ...
    Write-Host ("  Rollback to    : " + $previousRuntime + "   (deploy-litellm-linux.ps1 -Action Rollback)")
    Write-Host ''
...
Assert-LinuxPlatform
$script:TargetTriple = Get-LinuxTargetTriple

# Resolve the box directory here (not in the param default): PowerShell leaves $PSScriptRoot
# empty during param binding when launched with `pwsh -File`, as start-litellm.sh does.
if ([string]::IsNullOrWhiteSpace($ContainerRoot)) {
