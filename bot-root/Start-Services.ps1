...
$binariesFolder       = Join-Paths @($baseFolder, "grid", "binaries")
$configurationsFolder = Join-Paths @($baseFolder, "grid", "configurations")
$dotnetFolder         = Join-Paths @($baseFolder, "..", "runtime", "dotnet-windows")
$driversFolder        = Join-Paths @($baseFolder, "grid", "drivers-windows")
...
$chromeDriverExecutable = Join-Paths @($driversFolder, "chromedriver")
...
Write-Host "Registering Edge Node on port 5553..."
Start-Process `
    -FilePath         $javaExecutable `
    -ArgumentList     "-jar $($seleniumExecutable) node --hub $($HubUri) --config $(Join-Paths -Paths @($configurationsFolder, 'edge-node.toml')) --port 5553" `
    -WorkingDirectory $binariesFolder
