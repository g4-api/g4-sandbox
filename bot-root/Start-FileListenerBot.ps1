...
if ($Docker) {
    try {
        ...
        $cmdLines += " -p $($botConfiguration.Endpoints.CallbackPort):$($botConfiguration.Endpoints.CallbackPort) --name `"$($botConfiguration.Metadata.BotName)-$([guid]::NewGuid())`" g4-file-listener-bot:latest"
...
$inputDirectory = $bot.Configuration.Directories.BotInputDirectory
...
while (-not $bot.CallbackJob.AsyncResult.IsCompleted -and $bot.CallbackJob.Runner.InvocationStateInfo.State -eq 'Running') {
    try {
        ...
        $nextFile = Get-NextFile `
            -InputDirectory   $bot.Configuration.Directories.BotInputDirectory `
            -ArchiveDirectory $bot.Configuration.Directories.BotArchiveDirectory `
            -Session          $session `
            -Extensions       'csv','json'
